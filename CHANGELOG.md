...
- **GPT-5.6 and Bedrock Runtime support.** The LiteLLM gateway now routes Sol,
  ...
  (`global.openai.gpt-5.6-*`) and refreshes the Runtime bearer token used by
  active requests. Native Codex and AgentCore examples retain their current
  Mantle connector IDs until those built-in integrations support Bedrock
  Runtime directly.
