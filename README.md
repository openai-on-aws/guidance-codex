...
`-- YES -> Want to operate the gateway data plane?
           |-- YES -> LiteLLM on ECS
           `-- NO  -> Portkey hosted Bedrock Mantle
```
...
| **[Portkey](docs/QUICKSTART_LLM_GATEWAY_PORTKEY.md)** | Hosted; scoped AWS role | Workspace key/JWT telemetry | Vendor-managed policy, routing, and analytics |
...
- **Portkey Gateway** → [Hosted Bedrock Mantle evaluation](docs/QUICKSTART_LLM_GATEWAY_PORTKEY.md)
